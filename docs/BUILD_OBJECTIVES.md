...
| Fortomnia Apple Watch app | In progress | HealthKit data produced by Apple Watch can be read on iPhone through Apple Health; versioned workout-transfer and idempotent offline-action contracts exist, with a native iPhone WatchConnectivity bridge for snapshot delivery, offline fallback, acknowledgements, and connection state | Add the SwiftUI watchOS target with planned-session start, current exercise, set logging, load/reps/RPE or RIR, timers, heart rate, next exercise, completion, durable offline queueing, and phone reconciliation |
