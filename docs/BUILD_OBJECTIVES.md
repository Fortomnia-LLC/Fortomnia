...
- Routed Android activity and sleep summaries through Health Connect aggregates so system-managed user source priorities prevent overlapping apps from inflating recovery totals.
...
| Android health | In progress | Cross-provider types include `health_connect`; the existing `fortomnia-health` module includes availability, permissions, normalized reads, provider-aware cache reconciliation, Recovery UI integration, privacy-rationale routing, and aggregate activity/sleep summaries that honor user-selected Health Connect source priorities | Add incremental changes, background ingestion, write-back, and physical-device validation |
