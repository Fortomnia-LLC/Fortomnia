...
| Offline-first workout data | P0 | Complete foundation | Active workouts and queued mutations persist in a versioned AsyncStorage working copy, survive process restarts and connectivity loss, replay in stable order, and expose shared sync/retry status | Validate airplane-mode and process-termination behavior on physical iPhone and Watch hardware, then extend the proven contract to other user-owned domains |
| Shared synchronization and conflict model | P0 | Complete foundation | Workout sets now use stable mutation IDs, optimistic revisions, server-side atomic compare-and-apply, durable tombstones, deterministic stale-write rejection, and idempotent retry across the phone/Watch/Supabase path | Validate multi-device conflicts on physical hardware, add tombstone retention/compaction operations, and incrementally adopt the contract for nutrition, supplements, forms, and collaboration |
| Database ownership, RLS, constraints, and indexes | P0 | In progress | All current user-owned tables have live RLS and ownership indexes; migration contract tests now require reviewed ownership policies, per-operation isolation, update `with check`, and indexed ownership columns | Add authenticated two-user integration tests against an isolated database branch; enable leaked-password protection if the Auth plan supports it; complete least-privilege API and sensitive health/nutrition constraint reviews |
| Backward-compatible schema and API evolution | P0 | In progress | Supabase migrations are version controlled and the migration policy now requires additive rollout, compatibility windows, backfill, recovery plans, and explicit review markers for destructive DDL | Add automated ephemeral-database forward/rollback validation; version remaining sync and server contracts; remove old fields only after an explicit compatibility window |
...
| Architecture and synchronization test suite | P0 | In progress | Unit tests, TypeScript validation, Expo Doctor, migration checks, repository contracts, offline restart/replay, optimistic revisions, tombstone guards, and milestone quality workflows exist | Add an ephemeral-database integration suite for concurrent clients and RLS isolation plus auth/session recovery, backward-compatible migration, wearable normalization, and deterministic fallback coverage |
...
- Verified the live Supabase schema has RLS and ownership indexes on every current user-owned table, then added migration contract tests that prevent unreviewed tables, incomplete ownership policies, unindexed RLS columns, deprecated role checks, unsafe public definer functions, and unreviewed destructive DDL.
- Documented the required additive migration, backfill, compatibility-window, and recovery sequence for future schema evolution.
- Added anchored HealthKit reads with stable sample identities, deletion reconciliation, bounded local caching, background observer delivery, and visible last-sync freshness.
...
### Completed increments — 2026-09-15

- Expanded the iPhone and Lock Screen widget snapshot with the most recently updated saved workout, exercise count, and active training location while keeping sensitive health values out of the shared payload.
- Improved active-workout widget progress to show completed versus planned sets and refreshed widget data when the app returns to the foreground.
- Added a dedicated Home Screen and Lock Screen Recovery widget that reuses Fortomnia's deterministic readiness engine, publishes only the derived score/band/date, redacts values on protected surfaces, and routes missing check-ins to the existing check-in flow.
...
- Added permission-aware supplement barcode scanning by extending the existing camera pattern, with editable serving and ingredient facts, explicit community-data provenance, manual-entry fallback, bounded metadata, and per-user duplicate prevention.
...
| Health signals | Complete foundation | Sleep, heart rate, resting heart rate, HRV, steps, active energy, workouts, weight, and body-fat types are represented; incremental/background ingestion and freshness states are implemented | Validate units and source precedence on physical devices |
...
| Fortomnia Apple Watch app | In progress | Native SwiftUI watchOS target, versioned workout transfer, offline action queue, acknowledgements, and iPhone WatchConnectivity reconciliation are implemented; the signed beta build is uploaded and waiting for external TestFlight review | Complete physical Watch testing, verify workout/heart-rate behavior and offline reconciliation, and close issues found in beta |
| Workout Live Activities | Complete foundation | ActivityKit bridge plus Lock Screen and Dynamic Island UI show the current exercise, set progress, and active rest countdown with a deep link back to the workout | Compile through hosted iOS CI, then validate start/update/end behavior, device lock states, Dynamic Island layouts, and battery impact on physical iPhone hardware |
...
| iPhone and Lock Screen widgets | In progress | WidgetKit Home Screen and Lock Screen target, shared App Group, active-workout progress, next saved workout, active training location, a dedicated recovery/readiness widget, privacy redaction, stale/empty handling, system-limited refresh, and Fortomnia deep links are implemented | Validate the expanded states on physical iPhone/Lock Screen, add explicit next-workout selection/scheduling, then add reminder states |
...
| Background workout alerts | Complete foundation | Existing Expo notification preferences now include one-shot local rest-timer notifications, a high-importance Android channel, sound, cancellation, and safe workout deep-link handling | Validate permission-denied, app-backgrounded, app-terminated, timer-restart, and notification-tap behavior on physical iPhone and Android devices; reserve remote push for server-originated events |
...
| Gym profiles and session equipment | In progress | Users can save, edit, and safely delete home, gym, hotel, outdoor, and other locations with equipment and notes; active-location selection drives equipment-aware program generation; new workout sessions retain a creation-time location/equipment snapshot in history | Bridge specialty implements into each location and validate travel workflows on devices |
...
| Working-template exercise editing | Complete foundation | Existing templates expose a prominent **Add exercise** action backed by the searchable/filterable exercise picker, proactive duplicate exclusion, append ordering, explicit save/cancel controls, edit/delete/reorder actions, and regression tests | Validate the editing flow on physical iPhone and Android hardware and close issues found during beta |
| Supplement barcode scanning | Complete foundation | Supplement tracking now includes permission-aware UPC/EAN scanning, editable serving and ingredient facts, explicit Open Food Facts provenance, manual-entry fallback, bounded metadata, and per-user duplicate prevention | Validate product coverage and camera behavior on physical iPhone and Android devices; keep label review mandatory and add another licensed product source only if measured lookup coverage requires it |
| Amazon storefront integration | Conditional | Fortomnia product and supplement experiences can provide contextual commerce entry points | Validate user value, regional availability, affiliate/brand-account requirements, App Store and Play billing/linking rules, disclosures, attribution, privacy, and maintainability before implementation; keep purchasing optional and separate from health guidance |
| GymRadar-informed expansion and account connection | Required with source constraint | Existing Fortomnia catalog and specialty taxonomy are the destination | Use GymRadar to study category coverage and identify omissions. Investigate optional user-authorized GymRadar account connection/import only through a supported API or partnership, with explicit consent, disconnect/delete controls, and documented data provenance. Do not scrape/copy its listings, reviews, photos, account data, or proprietary content without an API, partnership, or explicit reuse license |
| Android release optimization (R8/DEX) | Required | Google Play reports DEX obfuscation below its recommended threshold; production builds already use the Android App Bundle pipeline | Use Android R8 Analyzer to identify redundant, broad, or subsumed keep rules; enable and tune shrinking, optimization, obfuscation, and resource removal without breaking Expo, React Native, Supabase, Health Connect, or native modules; compare bundle size/startup behavior; run automated and physical-device regression tests; ship through internal testing before production; satisfy the Google Play requirement before February 2027 |
...
4. Extend `exercises` plus current exercise/implement/event mappings for single-limb coverage and template editing; do not create a replacement exercise library.
