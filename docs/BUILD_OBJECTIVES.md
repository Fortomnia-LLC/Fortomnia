...
Status legend: **Complete foundation** = usable groundwork exists but may still need production hardening; **In progress** = partially implemented; **Required** = accepted objective not yet implemented; **Later** = intentionally outside the current milestone.
