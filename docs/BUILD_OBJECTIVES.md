...
| Android health | In progress | Cross-provider types include `health_connect`; the existing `fortomnia-health` module now includes a Health Connect availability, permission, and normalized-read bridge | Connect the Recovery UI, add durable connection/cache state, incremental changes, background ingestion, source precedence, privacy-rationale routing, write-back, and physical-device validation |
...
| Gym profiles and session equipment | In progress | Users can save home, gym, hotel, outdoor, and other locations with equipment and notes, choose one active location, and generate programs from that location's equipment | Add location editing, bridge specialty implements into each location, attach the chosen location to workout history, and validate travel workflows on devices |
